...
To build a ROS node using dv-processing, add `dv_ros2_messaging` and `dv_ros2_msgs` as a dependency in the find catkin
...
When writing a node, make sure to include the `dv_ros2_messaging/messaging.hpp` header before any ROS headers are
included. Use the `DV_ROS2_MSGS` macro when declaring types to avoid compilation issues with the std allocator. E.g. if
you use sensor_msgs::Image type, just wrap the type declaration with the macro:

```c++
// Variable declaration
DV_ROS2_MSGS(sensor_msgs::Image) image;

// Or using an alias for the type
using ImageMessage = DV_ROS2_MSGS(sensor_msgs::Image);
ImageMessage image;
```
